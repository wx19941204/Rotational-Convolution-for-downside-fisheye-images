...
class DoubleRotDefConv(nn.Module):
    """(Rotational Convolution => [BN] => ReLU) * 2"""
...
class RotDefUp(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleRotDefConv(in_channels, out_channels, in_channels // 2)
        else:
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleRotDefConv(in_channels, out_channels)
